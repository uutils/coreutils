...
impl Uniq {
    pub fn print_uniq<R: Read, W: Write>(&self,
        ...
        for io_line in reader.lines() {
            ...
            if !lines.is_empty() &&
               self.cmp_key(&self.skip_fields(&lines[0])) !=
               self.cmp_key(&self.skip_fields(&line)) {
                let print_delimiter = delimiters == "prepend" ||
                                      (delimiters == "separate" && first_line_printed);
    ...
    fn print_lines<W: Write>(&self,
                             writer: &mut BufWriter<W>,
                             lines: &Vec<String>,
                             print_delimiter: bool)
                             -> bool {
        let mut first_line_printed = false;
        let mut count = if self.all_repeated {
            1
        } else {
            lines.len()
        };
        ...
        if self.all_repeated {
            for line in lines[1..].iter() {
                self.print_line(writer,
                                line,
                                count,
                                print_delimiter && !first_line_printed);
                first_line_printed = true;
                count += 1;
            }
        }
...
pub fn uumain(args: Vec<String>) -> i32 {
    ...
    if matches.opt_present("help") {
        ...
        println!("");
        print!("{}",
               opts.usage("Filter adjacent matching lines from INPUT (or standard input),\n\
