...
fn split_lines(bencher: Bencher) {
    ...
    bencher
        .with_inputs(|| {
            ...
            let args = get_bench_args(&[&"-l", &"1000", &file_path, &prefix]).into_iter();
            (output_dir, args)
        })
...
fn split_bytes(bencher: Bencher) {
    ...
    bencher
        .with_inputs(|| {
            let output_dir = TempDir::new().unwrap();
            let prefix = output_dir.path().join("x");
            let args = get_bench_args(&[&"-b", &"100K", &file_path, &prefix]).into_iter();
            (output_dir, args)
        })
        .bench_values(|(output_dir, args)| {
            black_box(uumain(args));
...
fn split_number_chunks(bencher: Bencher) {
    ...
    bencher
        .with_inputs(|| {
            let output_dir = TempDir::new().unwrap();
            let prefix = output_dir.path().join("x");
            let args = get_bench_args(&[&"-n", &"10", &file_path, &prefix]).into_iter();
            (output_dir, args)
        })
        .bench_values(|(output_dir, args)| {
            black_box(uumain(args));
...
fn split_numeric_suffix(bencher: Bencher) {
    ...
    bencher
        .with_inputs(|| {
            let output_dir = TempDir::new().unwrap();
            let prefix = output_dir.path().join("x");
            let args = get_bench_args(&[&"-d", &"-l", &"500", &file_path, &prefix]).into_iter();
            (output_dir, args)
        })
        .bench_values(|(output_dir, args)| {
            black_box(uumain(args));
