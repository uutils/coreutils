    ...
    match writer.write(bytes) {
            ...
            if let Some((filename, error_reported)) = error_context {
                match writer.flush() {
                    ...
                    Err(e) => {
                        ...
                        Err(io::Error::new(
                            ErrorKind::Other,
                            translate!("split-error-flush-already-reported"),
                        ))
                    }
                }
            } else {
        ...
        Err(e) => {
            if let Some((filename, error_reported)) = error_context {
                ...
                Err(io::Error::new(
                    ErrorKind::Other,
                    translate!("split-error-write-already-reported"),
                ))
            } else {
                Err(e)
            }
            ...
            if self.num_bytes_remaining_in_current_chunk == 0 {
                ...
                if let Err(e) = self.inner.flush() {
                    ...
                    return Err(io::Error::new(
                        ErrorKind::Other,
                        translate!("split-error-flush-before-file-switch"),
                    ));
                }
    ...
    fn flush(&mut self) -> io::Result<()> {
        match self.inner.flush() {
            ...
            Err(e) => {
                ...
                Err(io::Error::new(
                    ErrorKind::Other,
                    translate!("split-error-flush-in-chunk-writer"),
                ))
            }
        }
    }
