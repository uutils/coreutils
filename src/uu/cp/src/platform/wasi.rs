...
fn to_timespec(time: SystemTime) -> io::Result<Timespec> {
    match time.duration_since(UNIX_EPOCH) {
        ...
        Err(error) => {
            ...
            let seconds: i64 = duration.as_secs().try_into().map_err(io::Error::other)?;
            let nanoseconds = duration.subsec_nanos();
            let (tv_sec, tv_nsec) = if nanoseconds == 0 {
                (-seconds, 0)
            } else {
                (
                    seconds
                        ...
                        .ok_or_else(|| io::Error::other("timestamp is out of range"))?,
                    (1_000_000_000 - nanoseconds) as i32,
                )
            };
...
mod tests {
    ...
    #[test]
    fn timespec_preserves_pre_epoch_fractional_seconds() {
        let time = UNIX_EPOCH - Duration::new(1, 250_000_000);

        let timespec = to_timespec(time).unwrap();

        assert_eq!(timespec.tv_sec, -2);
        assert_eq!(timespec.tv_nsec, 750_000_000);
    }
