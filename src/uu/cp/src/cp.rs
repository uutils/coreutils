...
fn show_error_if_needed(error: &CpError) {
    match error {
        // NotAllFilesCopied: Need to return an error code
        // Skipped: touch a b && echo "n"|cp -i a b && echo $? should return an error
        // XattrErrorsReported: each failing attribute was already reported on stderr by `copy_xattrs*`
        CpError::NotAllFilesCopied | CpError::Skipped(_) | CpError::XattrErrorsReported => {}
