...
impl Who {
    ...
    fn print_runlevel(&self, ut: &Utmpx) {
        ...
        let comment = translate!("who-runlevel-last", "last" => (if last == 'N' { 'S' } else { 'N' }).to_string(),);
    ...
    fn print_user(&self, ut: &Utmpx) -> UResult<()> {
        ...
        let s = if self.do_lookup {
            ut.canon_host().map_err_context(|| {
                let host = ut.host();
                translate!("who-canonicalize-error", "host" => host.split(':').next().unwrap_or(&host).quote().to_string(),)
                .to_string()
            })?
        } else {
            ut.host()
