...
pr-error-invalid-pages-argument = argument --pages invalide { $arg }
pr-error-invalid-pages-range = plage de pages invalide { $range }
pr-error-invalid-plus-argument = argument + invalide { $arg }
pr-error-value-too-large = { $desc } : { $arg } : Valeur trop grande pour le type de données défini
pr-error-numerical-result-out-of-range = { $desc } : { $arg } : Résultat numérique hors limites
pr-error-invalid-argument = { $desc } : { $arg }
