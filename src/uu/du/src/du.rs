                ...
                Ok(entry) => {
                    match Stat::new(&entry.path(), Some(&entry), options) {
                        ...
                        Err(e) => print_tx.send(Err(e.map_err_context(|| {
                            translate!("du-error-cannot-access", "path" => entry.path().quote().to_string(),)
                        })))?,
                    }
                }
...
enum DuError {
    ...
    #[error("{}", translate!("du-error-invalid-time-arg"))]
    InvalidTimeArg,
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let size_format = if matches.get_flag(options::HUMAN_READABLE) {
        ...
        if block_size == 0 {
            return Err(std::io::Error::other(translate!("du-error-invalid-block-size-argument", "option" => options::BLOCK_SIZE, "value" => block_size_str.map_or("???BUG", |v| v).quote().to_string(),))
            .into());
        }
        SizeFormat::BlockSize(block_size)
    };
        ...
        if let Ok(stat) = Stat::new(&path, None, &traversal_options) {
            ...
            print_tx
                .send(Err(USimpleError::new(
                    1,
                    translate!("du-error-cannot-access-no-such-file", "path" => path.to_string_lossy().quote().to_string(),),
                )))
                .map_err(|e| USimpleError::new(1, e.to_string()))?;
        }
    }
