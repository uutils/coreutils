...
fn parse_date<S: AsRef<str> + Clone>(
    ...
    let normalized = normalize_iso8601_12hour_time(input_str);
    ...
    if let Some(zoned) = try_parse_with_abbreviation(&normalized) {
        if dbg_opts.debug {
            eprintln!(
    ...
    match parse_datetime::parse_datetime_at_date(now.clone(), &normalized) {
