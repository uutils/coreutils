...
fn rewrite_european_date(input: &str, now: &Zoned) -> String {
    ...
    // Only the first European-looking token is rewritten; anything else is
    // left untouched so times (`5:15:30.25`), timezone suffixes and relative
    // items still parse.
    let found = tokens.iter().enumerate().find_map(|(index, token)| {
        let (day, month, inline_year) = european_date_token(token)?;
        ...
        Some((index, year_index, format!("{year}-{month:02}-{day:02}")))
    });

    let Some((index, year_index, iso)) = found else {
        return input.to_string();
    };
...
fn parse_date<S: AsRef<str>>(
    ...
    let input = s.as_ref();
    let input_str = rewrite_european_date(input, now);

    if dbg_opts.debug {
        let _ = writeln!(stderr(), "date: input string: {input}");
    }
...
mod tests {
    ...
    fn test_parse_european_date() {
        ...
        let parse =
            |input: &str| parse_date(input, &now, DebugOptions::new(false, false), false).unwrap();
