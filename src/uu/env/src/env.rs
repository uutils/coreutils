...
pub fn parse_args_from_str(text: &NativeIntStr) -> UResult<Vec<NativeIntString>> {
    crate::split_iterator::split(text).map_err(|e| match e {
        crate::parse_error::ParseError::BackslashCNotAllowedInDoubleQuotes { pos: _ } => {
            USimpleError::new(125, "'\\c' must not appear in double-quoted -S string")
        }
        crate::parse_error::ParseError::InvalidBackslashAtEndOfStringInMinusS {
            pos: _,
            quoting: _,
        } => USimpleError::new(125, "invalid backslash at end of string in -S"),
        crate::parse_error::ParseError::InvalidSequenceBackslashXInMinusS { pos: _, c } => {
            USimpleError::new(125, format!("invalid sequence '\\{}' in -S", c))
        }
        crate::parse_error::ParseError::MissingClosingQuote { pos: _, c: _ } => {
            USimpleError::new(125, "no terminating quote in -S string")
        }
        crate::parse_error::ParseError::ParsingOfVariableNameFailed { pos, msg } => {
