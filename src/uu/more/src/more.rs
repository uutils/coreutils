...
struct Pager<'a> {
    input: InputType,
    file_size: Option<u64>, // Total size of the file in bytes (only for files)
...
impl<'a> Pager<'a> {
    ...
    fn handle_from_line(&mut self) -> UResult<()> {
        self.read_until_line(self.upper_mark)?;
        if self.upper_mark >= self.lines.len() {
            write!(
                self.stdout,
    ...
    fn read_until_line(&mut self, target_line: usize) -> UResult<()> {
        // Check if already read enough lines
        if self.lines.len() > target_line {
            return Ok(());
        }
        ...
        while self.lines.len() <= target_line {
            ...
            let current_position =
                self.cumulative_line_sizes.last().unwrap_or(&0) + bytes_read as u64;
            self.cumulative_line_sizes.push(current_position);
    ...
    fn get_line(&mut self, index: usize) -> Option<&String> {
        if self.read_until_line(index).is_ok() {
...
mod tests {
    ...
    fn test_squeeze() {
        ...
        // load all lines
        pager.read_until_line(7).unwrap();
