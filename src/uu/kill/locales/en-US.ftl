...
kill-after-help-windows = Windows notes:
  ...
  and STOP are not supported. Permissions come from your current token, with
  SeDebugPrivilege enabled when it is held, so run elevated to reach processes
  a standard token cannot signal. Protected (anti-malware) processes cannot be
  terminated at all.
