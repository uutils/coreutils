...
pub trait ChecksumCommand {
    ...
    fn with_binary(self, require_untagged: bool) -> Self;

    fn with_text(self, require_untagged: bool, is_default: bool) -> Self;
...
impl ChecksumCommand for Command {
    ...
    fn with_binary(self, require_untagged: bool) -> Self {
        let mut arg = Arg::new(options::BINARY)
            .long(options::BINARY)
            .short('b')
            .hide(true)
            .action(ArgAction::SetTrue)
        ...
        if require_untagged {
            arg = arg.requires(options::UNTAGGED);
        };
