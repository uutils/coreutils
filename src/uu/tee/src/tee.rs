...
impl MultiWriter {
    ...
    /// not buffered which is POSIX requirement.
    pub fn copy_unbuffered<R: Read>(&mut self, mut input: R) -> Result<()> {
        // todo: support splice() and tee() zero-copy on Linux at here for performance
        // keep this fn at here since splice() needs raw fd

        // The implementation for this function is adopted from the generic buffer copy implementation from std
        // <https://github.com/rust-lang/rust/blob/e408947bfd200af42db322daf0fadfe7e26d3bd1/library/std/src/io/copy.rs#L271>

        // Use buffer size from std <https://github.com/rust-lang/rust/blob/e408947bfd200af42db322daf0fadfe7e26d3bd1/library/std/src/sys/io/mod.rs#L72>
        ...
        let mut buffer = vec![0u8; 4 * BUF_SIZE]; // allocation is deferred by heap for smaller input
        loop {
            match input.read(&mut buffer) {
                Ok(0) => return Ok(()), // end of file
                Ok(received) => {
