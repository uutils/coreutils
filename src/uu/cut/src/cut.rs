...
///
/// Ranges are sorted and disjoint, so a single forward cursor over the line's
/// characters serves every range in order. Characters are decoded on the fly
/// (no per-line allocation) and only as far as the ranges actually reach, so a
/// bounded selection like `-c 5-30` never decodes the tail of a long line.
fn cut_characters<R: Read, W: Write>(
    ...
    let result = buf_in.for_byte_record(newline_char, |line| {
        // `pos` is the byte offset of the next character; `char_no` is the count
        // of characters already consumed, so the next one is 1-based `char_no + 1`.
        let mut pos = 0;
        let mut char_no = 0;
        let mut print_delim = false;
        for &Range { low, high } in ranges {
            // Skip forward to the first character of this range.
            while char_no + 1 < low && pos < line.len() {
                pos += mb_char_len(&line[pos..]).clamp(1, line.len() - pos);
                char_no += 1;
            }
            // `low` lies past the end of the line: no later range can match either.
            if char_no + 1 < low {
                break;
            }
            let start = pos;
            // Consume the characters selected by this range (clamped to line end).
            while char_no < high && pos < line.len() {
                pos += mb_char_len(&line[pos..]).clamp(1, line.len() - pos);
                char_no += 1;
            }
            if pos == start {
