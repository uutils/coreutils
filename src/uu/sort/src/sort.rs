...
impl KeySettings {
    ...
    fn check_compatibility(
        ...
        if matches!(
            ...
            if dictionary_order {
                return Err(
                    translate!("sort-options-incompatible", "opt1" => "d", "opt2" => mode.get_short_name().unwrap().to_string(),),
                );
            } else if ignore_non_printing {
                return Err(
                    translate!("sort-options-incompatible", "opt1" => "i", "opt2" => mode.get_short_name().unwrap().to_string(),),
                );
            }
        }
        Ok(())
    ...
    fn set_sort_mode(&mut self, mode: SortMode) -> Result<(), String> {
        if self.mode != SortMode::Default && self.mode != mode {
            return Err(
                translate!("sort-options-incompatible", "opt1" => self.mode.get_short_name().unwrap().to_string(), "opt2" => mode.get_short_name().unwrap().to_string()),
            );
        }
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
            ...
            Err(e) => {
                let error_message = if *e.kind() == IntErrorKind::PosOverflow {
                    #[cfg(target_os = "linux")]
                    {
                        show_error!(
                            "{}",
                            translate!("sort-batch-size-too-large", "arg" => n_merge.quote())
                        );

                        translate!("sort-maximum-batch-size-rlimit", "rlimit" => get_rlimit()?)
                    }
    ...
    if files.is_empty() {
        ...
        return Err(UUsageError::new(
            2,
            translate!("sort-extra-operand-not-allowed-with-c"),
        ));
    }
