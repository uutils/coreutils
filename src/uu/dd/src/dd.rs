...
#[cfg(any(target_os = "linux", target_os = "android"))]
fn allocate_aligned_buffer(size: usize) -> io::Result<Vec<u8>> {
    Vec::<u8>::new().try_reserve(size)?;
    ...
    Ok(unsafe { Vec::from_raw_parts(ptr, 0, size) })
}
...
fn allocate_aligned_buffer(size: usize) -> io::Result<Vec<u8>> {
    let mut buf = Vec::new();
    buf.try_reserve(size)?; // try_with_capacity is unstable https://github.com/rust-lang/rust/issues/91913
    Ok(buf)
}
...
mod tests {
    ...
    fn test_aligned_buffer_allocation() {
        ...
        let buf = super::allocate_aligned_buffer(4096).unwrap();
    ...
    fn test_aligned_buffer_various_sizes() {
        ...
        for size in sizes {
            let buf = super::allocate_aligned_buffer(size).unwrap();
            let ptr = buf.as_ptr() as usize;
            assert_eq!(
                ptr % page_size,
                0,
    ...
    fn test_aligned_buffer_initialization() {
        ...
        let buf = super::allocate_aligned_buffer(1024).unwrap();
