...
impl FilterMode {
    ...
    fn from(matches: &ArgMatches) -> UResult<Self> {
        let zero_term = matches.get_flag(crate::options::ZERO_TERM);
        let mode = if let Some(arg) = matches.get_one::<String>(crate::options::BYTES) {
            match parse_num(arg) {
        ...
        } else if let Some(arg) = matches.get_one::<String>(crate::options::LINES) {
            match parse_num(arg) {
                Ok(signum) => {
...
impl Settings {
    ...
    pub fn from(matches: &clap::ArgMatches) -> UResult<Self> {
        ...
        let follow_retry = matches.get_flag(crate::options::FOLLOW_RETRY);
        // We don't need to check for occurrences of --retry if -F was specified which already sets
        // retry
        let retry = follow_retry || matches.get_flag(crate::options::RETRY);
        let follow = match (
            follow_retry,
            matches
                .get_one::<String>(crate::options::FOLLOW)
                .map(|s| s.as_str()),
        ) {
            ...
            (true, Some(_))
                ...
                if matches.index_of(crate::options::FOLLOW_RETRY) > matches.index_of(crate::options::FOLLOW) =>
            {
                Some(FollowMode::Name)
            }
        ...
        let mut settings: Self = Self {
            follow,
            retry,
            use_polling: matches.get_flag(crate::options::USE_POLLING),
            mode: FilterMode::from(matches)?,
            verbose: matches.get_flag(crate::options::verbosity::VERBOSE),
            presume_input_pipe: matches.get_flag(crate::options::PRESUME_INPUT_PIPE),
            ..Default::default()
        };

        if let Some(source) = matches.get_one::<String>(crate::options::SLEEP_INT) {
        ...
        if let Some(s) = matches.get_one::<String>(crate::options::MAX_UNCHANGED_STATS) {
        ...
        if let Some(pid_str) = matches.get_one::<String>(crate::options::PID) {
            match pid_str.parse() {
                Ok(pid) => {
        ...
        settings.verbose =
            settings.inputs.len() > 1 && !matches.get_flag(crate::options::verbosity::QUIET);
...
pub fn parse_args(args: impl uucore::Args) -> UResult<Settings> {
    ...
    let clap_args = crate::uu_app().try_get_matches_from(args_vec.clone());
...
mod tests {
    ...
    fn test_parse_settings_follow_mode_and_retry(
        ...
        let settings =
            Settings::from(&crate::uu_app().no_binary_name(true).get_matches_from(args)).unwrap();
