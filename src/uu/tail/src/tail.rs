...
static OPT_FOLLOW_RETRY: &str = "F";
static OPT_LINES: &str = "lines";
static OPT_PID: &str = "pid";
static OPT_QUIET: &str = "quiet";
static OPT_RETRY: &str = "retry";
...
pub fn uumain(args: impl uucore::Args) -> i32 {
    ...
    let app = App::new(executable!())
        ...
        .arg(
            Arg::with_name(OPT_FOLLOW)
                .short("f")
                .long(OPT_FOLLOW)
                .help("Equivalent to --follow --retry"),
        )
        .arg(
            Arg::with_name(OPT_FOLLOW_RETRY)
                .short(OPT_FOLLOW_RETRY)
    ...
    if matches.is_present(OPT_FOLLOW_RETRY) {
        settings.follow = true;
        settings.retry = true;
    } else {
        settings.follow = matches.is_present(OPT_FOLLOW);
        settings.retry = matches.is_present(OPT_RETRY);
    }
