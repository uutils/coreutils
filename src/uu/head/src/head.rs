...
impl PrintError {
    /// Wrap a read `io::Error` into a `PrintError::ReadError`, stripping the errno suffix.
    fn read(e: io::Error) -> Self {
        Self::ReadError(io::Error::new(e.kind(), uucore::error::strip_errno(&e)))
    }

    /// Wrap a write `io::Error` into a `PrintError::WriteError`, adding the stdout context.
    fn write(e: io::Error) -> Self {
        Self::WriteError(wrap_in_stdout_error(e))
    }

    fn into_uu_error(self, name: PathBuf) -> Box<dyn UError> {
        match self {
            Self::ReadError(err) => HeadError::Io { name, err }.into(),
            Self::WriteError(err) => err.into(),
        }
    }
}
...
fn print_n_bytes(input: impl Read + AsFd, n: u64) -> Result<u64, PrintError> {
    let mut out = io::stdout();
    let res = uucore::pipes::send_n_bytes(input, &out, n).map_err(PrintError::write);
...
fn print_n_bytes(input: impl Read, n: u64) -> io::Result<u64, PrintError> {
    ...
    loop {
        let bytes_read = reader.read(&mut buf).map_err(PrintError::read)?;
...
fn print_n_lines(input: &mut impl io::BufRead, n: u64, separator: u8) -> Result<u64, PrintError> {
...
fn print_but_last_n_bytes(mut input: impl Read, n: u64) -> Result<u64, PrintError> {
...
fn print_but_last_n_lines(mut input: impl Read, n: u64, separator: u8) -> Result<u64, PrintError> {
    let stdout = io::stdout();
    let mut stdout = stdout.lock();
    if n == 0 {
        return io::copy(&mut input, &mut stdout).map_err(PrintError::write);
    }
    let mut bytes_written: u64 = 0;
    if let Some(n) = catch_too_large_numbers_in_backwards_bytes_or_lines(n) {
...
fn head_backwards_file(input: &mut File, options: &HeadOptions) -> Result<u64, PrintError> {
    let st = input.metadata().map_err(PrintError::read)?;
...
fn head_backwards_on_seekable_file(
    ...
    match options.mode {
        Mode::AllButLastBytes(n) => {
            let size = input.metadata().map_err(PrintError::read)?.len();
            if n >= size {
                Ok(0)
            } else {
...
fn head_file(input: &mut File, options: &HeadOptions) -> Result<u64, PrintError> {
    match options.mode {
    ...
    for file in &options.files {
        let res = if file == "-" {
            ...
            #[cfg(unix)]
            {
                ...
                if let Ok(current_pos) = current_pos {
                    ...
                    let bytes_read = match head_file(&mut stdin_file, options) {
                        Ok(n) => n,
                        Err(e) => return Err(e.into_uu_error("standard input".into())),
                    };
            ...
            #[cfg(not(unix))]
            {
                ...
                if let Err(e) = res {
                    return Err(e.into_uu_error("standard input".into()));
                }
            }
        ...
        if let Err(err) = res {
            ...
            return Err(err.into_uu_error(name));
        }
        first = false;
    }
