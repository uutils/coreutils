...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let command_name = command_values.next().unwrap();
        ...
        Err(e) => {
            return match e.kind() {
                std::io::ErrorKind::PermissionDenied => Err(USimpleError::new(
                    126,
                    translate!("stdbuf-error-permission-denied", "command" => command_name.quote()),
                )),
                std::io::ErrorKind::NotFound => Err(USimpleError::new(
                    127,
                    translate!("stdbuf-error-no-such-file", "command" => command_name.quote()),
                )),
