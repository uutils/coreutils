...
impl<'a> FileLines<'a> {
    ...
    fn match_anti_prefix(&self, line: &str) -> bool {
        ...
        match FileLines::match_prefix_generic(&self.opts.anti_prefix[..],
                                              line,
    ...
    fn match_prefix_generic(pfx: &str, line: &str, exact: bool) -> (bool, usize) {
        ...
        if !exact {
            ...
            while (i < line.len()) && line.char_at(i).is_whitespace() {
                i = match line.char_range_at(i) {
                    CharRange { ch: _ , next: nxi } => nxi,
                };
...
pub struct Paragraph {
    ...
    indent_end: usize, /* byte location of end of indent (in crown and tagged mode, only applies to 2nd
                        * line and onward) */
    pub mail_header: bool, /* we need to know if this is a mail header because we do word splitting
    ...
    fn is_mail_header(line: &FileLine) -> bool {
        ...
        if line.indent_end > 0 {
            ...
            if l_slice.starts_with("From ") {
                ...
                return l_slice[..colon_posn].chars().all(|x| {
                    match x as usize {
...
impl<'a> ParaWords<'a> {
    ...
    fn create_words<'r>(&'r mut self) {
        if self.para.mail_header {
            ...
            self.words.extend(self.para.lines.iter().flat_map(|x| x.split_whitespace()).map(|x| {
                WordInfo {
                    word: x,
                    word_start: 0,
                    word_nchars: x.len(), /* OK for mail headers; only ASCII allowed (unicode is
                                           * escaped) */
                    before_tab: None,
                    after_tab: 0,
            ...
            self.words.extend(if self.opts.crown || self.opts.tagged {
                ...
                WordSplit::new(self.opts,
                               &self.para.lines[0][self.para.indent_end..])
            });

            if self.para.lines.len() > 1 {
                let indent_end = self.para.indent_end;
                let opts = self.opts;
                self.words.extend(self.para
                                      .lines
                                      .iter()
                                      .skip(1)
...
impl<'a> Iterator for WordSplit<'a> {
    ...
    fn next(&mut self) -> Option<WordInfo<'a>> {
        ...
        let is_start_of_sentence = self.prev_punct &&
                                   (before_tab.is_some() || word_start_relative > 1);
