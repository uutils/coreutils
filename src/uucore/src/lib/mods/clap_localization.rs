...
fn find_long_opt<'a>(cmd: &'a Command, name: &str) -> Option<&'a clap::Arg> {
    for arg in cmd.get_arguments() {
        if arg.get_long() == Some(name) || arg.get_all_aliases().is_some_and(|a| a.contains(&name))
        {
            return Some(arg);
        }
    }
    let mut matches = cmd.get_arguments().filter(|a| {
...
fn find_short_opt(cmd: &Command, c: char) -> Option<&clap::Arg> {
    cmd.get_arguments().find(|arg| {
        arg.get_short() == Some(c) || arg.get_all_short_aliases().is_some_and(|a| a.contains(&c))
    })
}
