...
fn handle_display_errors(err: Error, util_name: &str) -> ! {
    match err.kind() {
        ErrorKind::DisplayHelp => {
...
fn handle_invalid_value_error(
    ...
    if let (Some(arg), Some(value)) = (invalid_arg, invalid_value) {
        ...
        if value.is_empty() {
            ...
            if matches!(err.kind(), ErrorKind::ValueValidation) {
...
pub fn handle_clap_error_with_exit_code(err: Error, util_name: &str, exit_code: i32) -> ! {
    ...
    match err.kind() {
        ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => {
            handle_display_errors(err, util_name);
        }
        ...
        kind if should_show_simple_help_for_clap_error(kind) => {
            ...
            if matches!(kind, ErrorKind::InvalidValue | ErrorKind::ValueValidation) {
                handle_invalid_value_error(err, util_name, maybe_colorize);
            }
...
mod tests {
    ...
    #[test]
    fn test_should_show_simple_help_for_clap_error() {
        assert!(should_show_simple_help_for_clap_error(
            ErrorKind::InvalidValue
        ));
        assert!(should_show_simple_help_for_clap_error(
            ErrorKind::InvalidSubcommand
        ...
        assert!(should_show_simple_help_for_clap_error(ErrorKind::NoEquals));
        assert!(should_show_simple_help_for_clap_error(ErrorKind::Io));
        assert!(should_show_simple_help_for_clap_error(ErrorKind::Format));
    ...
    #[test]
    fn test_localized_command_trait_basic() {
    ...
    #[test]
    fn test_localized_command_from_iter() {
    ...
    #[test]
    fn test_exit_code_selection_for_utilities() {
        let ls_utilities = ["ls", "dir", "vdir", "sort", "tty", "printenv"];

        for util in &ls_utilities {
            let expected_exit_code = match *util {
                "ls" | "dir" | "vdir" | "sort" | "tty" | "printenv" => 2,
                _ => 1,
            };
            assert_eq!(expected_exit_code, 2);
        }

        let other_utils = ["cat", "cp", "mv", "rm"];
    ...
    fn test_localization_setup() {
        ...
        let common_keys = ["common-error", "common-usage", "common-help-suggestion"];
        for key in &common_keys {
            let message = get_message(key);
            assert_ne!(message, *key, "Translation not found for key: {}", key);
        }

        let clap_keys = ["clap-error-unexpected-argument", "clap-error-invalid-value"];
