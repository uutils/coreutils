...
#[cfg(feature = "pipes")]
const TIME_T_POSIX_MAX: libc::time_t = 100_000_000;

/// Not all platforms support uncapped times (read: macOS). However,
/// we will conform to POSIX for portability.
/// <https://pubs.opengroup.org/onlinepubs/007904875/basedefs/sys/types.h.html#tag_13_67>
#[cfg(feature = "pipes")]
const SUSECONDS_T_POSIX_MAX: libc::suseconds_t = 1_000_000;
...
fn duration_to_timeval_elapsed(time: Duration, start: Instant) -> Option<TimeVal> {
    if time.is_zero() {
        ...
        // This code ensures we do not overflow on any platform.
        let mut time = TimeVal::new(
            time.as_secs()
                .try_into()
                .unwrap_or(libc::time_t::MAX)
                .saturating_sub_unsigned(elapsed.as_secs().try_into().unwrap_or_default()),
            time.subsec_micros()
                .saturating_sub(elapsed.subsec_micros().try_into().unwrap_or_default())
                .try_into()
                .unwrap_or_default(),
        );
        // Cap the time to POSIX specifications.
        if time.tv_sec() >= TIME_T_POSIX_MAX {
            time = TimeVal::new(TIME_T_POSIX_MAX - 1, time.tv_usec());
        }
        if time.tv_usec() >= SUSECONDS_T_POSIX_MAX {
            time = TimeVal::new(time.tv_sec(), SUSECONDS_T_POSIX_MAX - 1);
        }
        // Add some time to avoid underflow issues; 1 us.
        if time.tv_sec() == 0 && time.tv_usec() == 0 {
            time = TimeVal::new(0, 1);
        }
        Some(time)
    }
}
