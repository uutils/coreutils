...
pub fn is_hardlink(_source: &Path, _target: &Path) -> bool {
    false
}
...
#[cfg(unix)]
pub fn is_hardlink(source: &Path, target: &Path) -> bool {
...
mod tests {
    ...
    fn test_is_hardlink_same_file() {
        ...
        assert_eq!(is_hardlink(&path1, &path2), true);
    }

    #[cfg(unix)]
    #[test]
    fn test_is_hardlink_different_files() {
        ...
        assert_eq!(is_hardlink(&path1, &path2), false);
    }

    #[cfg(unix)]
    #[test]
    fn test_is_hardlink_hard_link() {
        let mut temp_file = NamedTempFile::new().unwrap();
        writeln!(temp_file, "Test content").unwrap();
        let path1 = temp_file.path();
