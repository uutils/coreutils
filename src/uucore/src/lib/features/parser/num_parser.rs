...
// spell-checker:ignore powf copysign inity infinit infs bigdecimal extendedbigdecimal biguint underflowed
...
fn construct_extended_big_decimal(
    ...
    let bd = if scale == 0 && exponent.is_zero() {
        ...
        // from_number_exp2 "only" supports i64 values. Just overflow/underflow if the value provided
        // is > 2**64 or < 2**-64.
        ...
        // Compute bd * 2 ** exponent.
        return Ok(ExtendedBigDecimal::from_number_exp2(bd, exponent));
    } else {
