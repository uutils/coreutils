...
fn total_physical_memory() -> Result<u128, SystemError> {
    #[cfg(target_family = "unix")]
    {
        ...
        let pages = unsafe { crate::libc::sysconf(crate::libc::_SC_PHYS_PAGES) };
        let page_size = unsafe { crate::libc::sysconf(crate::libc::_SC_PAGESIZE) };
...
fn total_physical_memory() -> Result<u128, SystemError> {
    ...
    unsafe {
        if crate::libc::sysctlbyname(
            name.as_ptr().cast::<crate::libc::c_char>(),
            ptr::addr_of_mut!(mem).cast::<crate::libc::c_void>(),
            ptr::addr_of_mut!(len),
            ptr::null_mut(),
            0,
        ) == 0
...
fn total_physical_memory() -> Result<u128, SystemError> {
    ...
    unsafe {
        if GlobalMemoryStatusEx(&mut mem_status) != 0 {
            Ok(mem_status.ullTotalPhys as u128)
        } else {
            Err(SystemError::IOError)
        }
    }
