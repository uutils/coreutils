...
mod tests {
    ...
    fn test_preserve_security_context() {
        ...
        if !is_selinux_enabled() {
            ...
            assert!(
                result.is_ok(),
                "preserve_security_context should succeed when SELinux is not enabled"
            );
            return;
        }
        ...
        if result.is_ok() {
            ...
            let dest_context =
                get_selinux_security_context(dest_path).expect("Failed to get destination context");
            assert!(
                dest_context.contains("tmp_t"),
                "Expected destination context to contain 'tmp_t', but got: {}",
                dest_context
            );
        } else {
            println!(
            ...
            if let Err(err) = preserve_result {
                println!("Context preservation failed as expected: {}", err);
            }
        }
