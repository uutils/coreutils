...
impl ChildExt for Child {
    ...
    fn wait_or_timeout(&mut self, timeout: Duration, ignore_term: bool) -> io::Result<TimeoutRet> {
        ...
        // A wait that ignores termination requests (the kill-after grace
        // period) also leaves the wake event out: any console event was
        // already forwarded once.
        let wake_event = if ignore_term { None } else { WAKE_EVENT.get() };
        ...
        if Some(index) == wake_index {
            if let Some(signal) = last_ctrl_signal() {
                return Ok(TimeoutRet::Interrupted(signal));
            }
        }
        // Timer expiry: the child is still running.
