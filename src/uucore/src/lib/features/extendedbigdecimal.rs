...
// spell-checker:ignore bigdecimal extendedbigdecimal biguint prec ilog muls
...
use bigdecimal::{BigDecimal, Context, num_bigint::BigUint};
...
pub fn pow_with_context(bd: &BigDecimal, exp: i64, ctx: &Context) -> BigDecimal {
    if exp == 0 {
        return 1.into();
    }
...
impl ExtendedBigDecimal {
    ...
    /// Create an `ExtendedBigDecimal` whose value is `number * 2**exponent`. Note that some
    /// precision may be lost for large exponent, to prevent the representation from blowing up.
    pub fn from_number_exp2<T: std::ops::Mul<BigDecimal, Output = BigDecimal>>(
        number: T,
        exponent: i64,
    ) -> Self {
        ...
        ExtendedBigDecimal::BigDecimal(number * pow2)
    }
}
