...
fn parse_chmod_inner(
    ...
    for raw_part in mode_string.split(',') {
        ...
        if mode_part.starts_with(|c: char| c.is_ascii_digit()) {
            return Err(ModeError {
                clause_start: start,
                ..ModeError::new(
        ...
        if mode_part.starts_with(['+', '-', '='])
            && mode_part[1..].chars().any(|c| c.is_ascii_digit())
        {
            new_mode =
                parse_numeric(new_mode, mode_part, considering_dir).map_err(|err| ModeError {
                    clause_start: start,
                    ..err
                })?;
            naive_mode = parse_numeric(naive_mode, mode_part, considering_dir)
                .expect("the clause parsed above");
        } else {
            new_mode =
                parse_symbolic(new_mode, mode_part, umask, considering_dir).map_err(|err| {
                    ModeError {
                        clause_start: start,
                        ..err
                    }
...
mod tests {
    ...
    #[test]
    fn test_parse_operator_numeric_in_list() {
        assert_eq!(parse_chmod(0o2755, "=7777,-5022", true, 0).unwrap(), 0o2755);
        assert!(parse_chmod(0, "+2000,-5022", false, 0).is_ok());
    }
