...
impl ModeError {
    ...
    /// under it.
    ///
    /// Every caller shows the same headline for every `ModeError` -- GNU
    /// itself only ever says "invalid mode" (with or without the operand,
    /// depending on the utility) -- so the detail that used to live there
...
pub fn parse_chmod(
    ...
    for raw_part in mode_string.split(',') {
        ...
        // An empty clause -- the whole mode, a leading, trailing or doubled
        // comma -- is not tolerated: GNU rejects `chmod u+rwx, f` the same
        // way it rejects `chmod , f`. `parse_symbolic` already reports that
        // correctly for an empty string, so it is not special-cased away
        // here.
