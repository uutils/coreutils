...
/// Convert a raw signal number to a validated `c_int`, real-time signals included.
///
/// Returns `None` for values that are not usable signals on this platform: zero
/// or negative numbers, out-of-range numbers, and platform gaps such as the
/// glibc-reserved 32/33 on Linux. Named signals are validated through rustix so
/// those gaps are rejected (matching GNU, which silently skips undefined
/// signals); real-time signals (`SIGRTMIN..=SIGRTMAX`) are not "named", so they
/// are accepted by range instead.
#[cfg(unix)]
pub fn signal_from_raw(signal: usize) -> Option<i32> {
    let raw = i32::try_from(signal).ok().filter(|&s| s > 0)?;
    if rustix::process::Signal::from_named_raw(raw).is_some() {
        return Some(raw);
    }
    realtime_signal_bounds()
        .and_then(|(rtmin, rtmax)| (rtmin..=rtmax).contains(&signal).then_some(raw))
}

/// Set a signal's disposition to `SIG_DFL` or `SIG_IGN`.
///
/// rustix deliberately does not wrap signal disposition (see its
/// `not_implemented::libc_internals`), leaving it to libc in a process that
/// links libc, so this goes straight to `libc::sigaction`. Using `sigaction`
/// with an empty mask (rather than `signal`) also lets callers target real-time
/// signals and gives consistent semantics across platforms.
#[cfg(unix)]
pub fn set_disposition(sig: libc::c_int, disposition: libc::sighandler_t) -> std::io::Result<()> {
    // SAFETY: a zeroed sigaction with an empty mask and SIG_DFL/SIG_IGN is a
    // valid disposition for any signal.
