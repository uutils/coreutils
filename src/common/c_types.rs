...
use self::libc::{c_char, c_int, uid_t, gid_t};
...
pub fn get_group_list(name: *const c_char, gid: gid_t) -> Vec<gid_t> {
    ...
    if unsafe { get_group_list_internal(name, gid, groups.as_mut_ptr(), &mut ngroups) } == -1 {
        ...
        unsafe {
            get_group_list_internal(name, gid, groups.as_mut_ptr(), &mut ngroups);
        }
    } else {
...
#[inline(always)]
unsafe fn get_group_list_internal(name: *const c_char,
                                  gid: gid_t,
                                  groups: *mut gid_t,
                                  grcnt: *mut c_int)
                                  -> c_int {
...
pub fn group(possible_pw: Option<c_passwd>, nflag: bool) {
    ...
    match groups {
        Err(errno) => crash!(1, "failed to get group list (errno={})", errno),
        Ok(groups) => {
