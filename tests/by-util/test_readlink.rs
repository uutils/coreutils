...
fn test_canonicalize() {
    ...
    let actual = ucmd.arg("-f").arg(".").run().stdout_str();
...
fn test_canonicalize_existing() {
    let (at, mut ucmd) = at_and_ucmd!();
    let actual = ucmd.arg("-e").arg(".").run().stdout_str();
    let expect = at.root_dir_resolved() + "\n";
    println!("actual: {:?}", actual);
    println!("expect: {:?}", expect);
    assert_eq!(actual, expect);
...
fn test_canonicalize_missing() {
    let (at, mut ucmd) = at_and_ucmd!();
    let actual = ucmd.arg("-m").arg(GIBBERISH).run().stdout_str();
...
fn test_long_redirection_to_current_dir() {
    ...
    let actual = ucmd.arg("-n").arg("-m").arg(dir).run().stdout_str();
    let expect = at.root_dir_resolved();
    println!("actual: {:?}", actual);
    println!("expect: {:?}", expect);
    assert_eq!(actual, expect);
...
fn test_long_redirection_to_root() {
    ...
    let actual = new_ucmd!().arg("-n").arg("-m").arg(dir).run().stdout_str();
