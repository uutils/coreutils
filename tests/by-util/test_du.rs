...
fn test_du_long_path_from_unreadable() {
    ...
    // Use a more reasonable depth for unit tests (20 levels instead of 52)
    let dir_name = "x".repeat(200); // 200-character directory name
    ...
    // Create an inaccessible directory to test from
    at.mkdir("inaccessible");

    // Save the current working directory
    let original_cwd = env::current_dir().unwrap();

    // Change to the inaccessible directory and make it unreadable
