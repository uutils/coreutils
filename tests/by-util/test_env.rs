...
fn test_env0_from_missing_file() {
    new_ucmd!()
        ...
        .fails_with_code(1)
        .no_stdout()
        .stderr_contains("env: no_such_env0: ");
}
...
fn test_env0_from_exec_preserves_env() {
    ...
    let res = scene
        ...
        .succeeds();
    res.no_stderr();
    let entries: Vec<&[u8]> = res
        .stdout()
        .split(|&b| b == 0)
        .filter(|e| !e.is_empty() && !e.starts_with(b"__CF_USER_TEXT_ENCODING="))
        .collect();
    assert_eq!(entries, vec![b"FOO=1".as_slice(), b"FOO=2".as_slice()]);
}
