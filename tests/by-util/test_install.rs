...
#[test]
#[cfg(target_os = "linux")]
fn test_install_set_owner_nonexistent_uid() {
    ...
    if let Ok(result) = run_ucmd_as_root(&ts, &[format!("-o{next_uid}").as_str(), "a", "b"]) {
        result.success();
...
#[test]
#[cfg(target_os = "linux")]
fn test_install_set_owner_nonexistent_gid() {
    let file = File::open("/etc/login.defs").unwrap();
    let reader = BufReader::new(file);
    ...
    if let Ok(result) = run_ucmd_as_root(&ts, &[format!("-g{next_gid}").as_str(), "a", "b"]) {
        result.success();
        assert!(at.file_exists("b"));
