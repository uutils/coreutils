...
fn test_final_stats_unspec() {
    new_ucmd!()
        .run()
        .stderr_matches(&Regex::new(r"0\+0 records in").unwrap())
        .stderr_matches(&Regex::new(r"0\+0 records out").unwrap())
        .stderr_matches(&Regex::new(r"0 bytes copied, 0\.\d+ s, 0 B/s").unwrap())
        .success();
}
