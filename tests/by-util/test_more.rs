...
fn run_more_with_pty(
    ...
    let mut child = ucmd
        // POSIXLY_CORRECT disables the default exit_on_eof so the pager stays alive
...
fn test_alive(args: &[&str]) {
    ...
    // Enough content to avoid hitting EOF on the initial page so the pager stays alive
    let content = (0..50)
        .map(|i| i.to_string())
        .collect::<Vec<String>>()
        .join("\n");
