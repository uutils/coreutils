...
complete -c sha256sum -s c -l check -d 'read hashsums from the FILEs and check them'
complete -c sha256sum -l tag -d 'create a BSD-style checksum'
complete -c sha256sum -s t -l text -d 'read in text mode (default)'
complete -c sha256sum -s q -l quiet -d 'don\'t print OK for each successfully verified file'
complete -c sha256sum -s s -l status -d 'don\'t output anything, status code shows success'
...
complete -c sha256sum -s z -l zero -d 'end each output line with NUL, not newline'
