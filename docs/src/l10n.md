...
It reads the `LC_ALL`, `LC_MESSAGES` and `LANG` environment variables (e.g., `fr-FR.UTF-8`) in that order, strips encoding, and parses the identifier. As for the C library, a variable that is set but empty counts as unset.

If parsing fails or none of them is set, it falls back to:
